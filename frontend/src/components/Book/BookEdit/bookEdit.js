...
const BookEdit= (props) =>
{
    ...
    const [formData,updateFormData]=React.useState({
        ...
        "author": props.authors.length>0 ? props.authors[0].id : 0,
        "availableCopies":0
    })
        ...
        <div className="row mt-5">
            <div className="col-md-5">
                <form onSubmit={onFormSubmit}>
                            ...
                            {props.categories.map((term) =>
                                {
                                    if(props.book.category !== undefined && props.book.category === term )
                                        return <option selected={props.book.category === term} value={term}>{term}</option>
                                    else
                            ...
                            {props.authors.map((term) =>

                                {
                                    if(props.book.author !== undefined && props.book.author.id === term.id )
                                        return <option selected={props.book.author.id === term.id} value={term.id}>{term.name} {term.surname}</option>
                                    else
                    ...
                    <div className="form-group">
                        <label htmlFor="availableCopies">Price</label>
                        <input type="text"
                               className="form-control"
                               id="availableCopies"
